...
empty :=
space := $(empty) $(empty)
COVER_PKGS      ?= $(shell go list ./... | grep -v '/internal/gen')
COVER_MODE      ?= atomic
...
.PHONY: proto
proto: ## Lint and regenerate protobuf code with buf
	@buf lint
	@buf generate

.PHONY: proto-breaking
proto-breaking: ## Check protobuf for breaking changes against git
